...
if(isset($_SESSION['utilisateur_id'])) {
    ...
    if(isset($_GET['annonce_id'])) {
        ...
        $sql = "SELECT UtilisateurID, PrixPropose FROM annoncecolistransport WHERE ID = ?";
        ...
        if ($resultat->num_rows > 0) {
            ...
            $annonce_besoin_transport_id = null;
            $livreur_id = $utilisateur_id; // Utiliser l'ID de l'utilisateur comme ID du livreur
            $requete_insert->bind_param("iiidi", $annonce_id, $annonce_besoin_transport_id, $user_id, $prix_convenu, $livreur_id);
            if ($requete_insert->execute()) {
                // Mettre à jour le statut de l'annonce dans la table annoncecolistransport
                $sql_update = "UPDATE annoncecolistransport SET Statut = 'termine' WHERE ID = ?";
